...
def grid_search(model, param, X_train, y_train):
    ...
    # define search
    search = GridSearchCV(pipeline, param_grid=param, scoring='neg_mean_absolute_error', 
                          cv=cv_inner, refit=True, return_train_score=True, n_jobs=30)
    # execute search
...
def get_feature_importance_plot(gridsearchcv_obj, modelname, features, iteration):
    ...
    selected_feature_indices = gridsearchcv_obj.best_estimator_.steps[0][1].top_features_[:gridsearchcv_obj.best_estimator_.steps[0][1].n_features_to_select]
    ...
    fig_feat.write_image("{}/{}_features_{}.svg".format(modelname, modelname, iteration),width=1000, height=1000)
...
def get_train_test_plot(test_errors,train_errors,labels, modelname):
    ...
    fig_val.add_trace(go.Violin(x0=modelname,
    ...
    fig_val.update_layout( xaxis_title = 'Model', 
    ...
    fig_val.write_image("{}/{}_validation.svg".format(modelname,modelname),width=1000, height=1000)
...
def get_actual_predict_plot(df_predictions, modelname):
    ...
    fig.add_trace(go.Scatter(x=df_predictions['lastphdospeak_actual'], 
                             y=df_predictions['lastphdospeak_predicted'], mode='markers',showlegend=False,
                            hovertext=df_predictions.index+'<br>Composition :'+ df_predictions.Composition))
                  
    fig.update_traces(marker=dict(size=10, color='#1878b6'))
    
    LR = LinearRegression().fit(np.array(df_predictions['lastphdospeak_actual']).reshape(-1,1),
                                df_predictions['lastphdospeak_predicted'])
    y_hat = LR.predict(np.array(df_predictions['lastphdospeak_actual']).reshape(-1,1))
    
    r2= round(LR.score(np.array(df_predictions['lastphdospeak_actual']).reshape(-1,1),
                    df_predictions['lastphdospeak_predicted']),3)
    
    fig.add_trace(go.Scatter(x=df_predictions['lastphdospeak_actual'], y=y_hat, mode='lines',
                             showlegend=False,line_color='#f57f1f'))
                  
    fig.update_layout( xaxis_title = '$\\text{Actual }\\omega\\text{ }(cm^{⁻1})$', 
    ...
    fig.write_image("{}/{}_predictions.pdf".format(modelname,modelname),width=1000, height=1000)
    fig.write_html("{}/{}_predictions.html".format(modelname,modelname),include_mathjax = 'cdn')
    
    df_predictions.to_csv('predictions_data.csv')
    
    logging.info('Saved models predictions overview data as csv and scatter plot for overall dataset')
